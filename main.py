...
class MyPlugin(Star):
    def __init__(self, context: Context):
        ...
        self.default_search_limit = 50  # 默认搜索结果显示数量
    ...
    @filter.command("mini_search")
    async def advanced_search(self, event: AstrMessageEvent):
        """高级搜索 - 使用方法：/mini_search 物品名称 [显示数量]"""
        message_str = event.message_str.strip()
        
        # 去除命令前缀
        if message_str.startswith('/mini_search'):
            message_str = message_str.replace('/mini_search', '', 1).strip()
        elif message_str.startswith('mini_search'):
            message_str = message_str.replace('mini_search', '', 1).strip()
        ...
        search_term = parts[0]
        limit = self.default_search_limit  # 使用设置的默认值
        ...
        if len(parts) > 1 and parts[1].isdigit():
            ...
            if limit > 500:
                limit = 500
                yield event.plain_result(f"⚠️ 单次最多显示 500 个结果，已自动调整为 500")
                return
        ...
        try:
            ...
            for item_name, item_data in self.items_cache.items():
                ...
                # 1. 直接包含匹配
                if search_keywords in item_name_lower or search_term in item_name:
                    found_items.append((item_name, item_data))
                    continue
                
                # 2. 中文分词模糊匹配
                if len(search_term) > 1:
                    all_chars_match = all(char in item_name for char in search_term)
                    if all_chars_match:
                        found_items.append((item_name, item_data))
                        continue
                    
                    for i in range(len(search_term) - 1):
                        two_char = search_term[i:i+2]
                        if two_char in item_name:
                            if (item_name, item_data) not in found_items:
                                found_items.append((item_name, item_data))
                            break
            ...
            if found_items:
                ...
                def match_score(item):
                    ...
                    if item_name == search_term:
                        score += 100
                    elif item_name.startswith(search_term):
                        score += 50
                    elif search_term in item_name:
                        score += 30
                    elif all(char in item_name for char in search_term):
                        score += 10
                    return score
                
                found_items.sort(key=match_score, reverse=True)
                
                result_text = f"🔍 高级搜索：**{search_term}**\n"
                result_text += f"找到 **{len(found_items)}** 个相关物品 (显示前 {min(limit, len(found_items))} 个):\n\n"
                
                for item_name, item_data in found_items[:limit]:
                    item_id = item_data['id']
                    item_type = item_data['type']
                    result_text += f"• {item_name} - ID: `{item_id}`【{item_type}】\n"
                
                if len(found_items) > limit:
                    result_text += f"\n\n💡 还有 {len(found_items) - limit} 个结果未显示\n"
                    result_text += f"📊 总共有 {len(found_items)} 个匹配物品"
                
                yield event.plain_result(result_text)
            else:
                yield event.plain_result(f"🔍 未找到 **{search_term}** 的相关信息\n\n💡 提示：请检查物品名称是否正确\n🌐 详细查询请访问：{self.base_url}")
            
            logger.info(f"用户高级搜索：{search_term}, 限制显示 {limit} 个，实际找到 {len(found_items)} 个结果")
            
        except Exception as e:
            logger.error(f"高级搜索失败：{e}")
            yield event.plain_result("❌ 搜索时发生错误，请稍后再试...")

    @filter.command("mini_setlimit")
    async def set_search_limit(self, event: AstrMessageEvent):
        """设置搜索结果显示数量 - 使用方法：/mini_setlimit 数量"""
        message_str = event.message_str.strip()
        
        # 去除命令前缀
        if message_str.startswith('/mini_setlimit'):
            message_str = message_str.replace('/mini_setlimit', '', 1).strip()
        elif message_str.startswith('mini_setlimit'):
            message_str = message_str.replace('mini_setlimit', '', 1).strip()
        
        if not message_str or not message_str.isdigit():
            yield event.plain_result(f"❌ 请输入有效的数字！\n\n✅ 正确用法：\n• /mini_setlimit 50\n• /mini_setlimit 100\n• /mini_setlimit 200\n\n📊 当前默认显示：**{self.default_search_limit}** 个结果")
            return
        
        limit = int(message_str)
        
        if limit < 1:
            yield event.plain_result("❌ 显示数量必须大于 0！")
            return
        
        if limit > 500:
            yield event.plain_result("⚠️ 单次最多显示 500 个结果，已自动设置为 500")
            limit = 500
        
        self.default_search_limit = limit
        yield event.plain_result(f"✅ 搜索结果显示数量已设置为 **{limit}** 个\n\n💡 提示：\n• 使用 `/mini_search` 时将默认显示 {limit} 个结果\n• 也可以在搜索时临时指定数量：`/mini_search 钻石 100`")
        logger.info(f"用户设置搜索结果显示数量为：{limit}")
...
3️⃣ **查看所有物品**
   ...
   功能：浏览所有物品的 ID 列表（支持翻页，每页 20 条）

4️⃣ **高级搜索（推荐）**
   指令：`/mini_search <物品名称> [数量]`
   示例：`/mini_search 钻石`、`/mini_search 工具 100`
   功能：自定义显示数量的模糊搜索（默认显示设置的值）

5️⃣ **设置显示数量**
   指令：`/mini_setlimit <数量>`
   示例：`/mini_setlimit 200`
   功能：设置搜索结果的默认显示数量（1-500）
...
- `/mini_search` 默认显示数量可通过 `/mini_setlimit` 设置
- 数据来源于第三方工具网站"""
